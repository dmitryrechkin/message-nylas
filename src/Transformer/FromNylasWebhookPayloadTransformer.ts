...
} from '../Type/NylasMessage';

import {
	type TypeMessageThread,
	type TypeMessage,
	type TypeMessageAttachment,
	messageThreadSchema
} from '@dmitryrechkin/message-core';
...
export class FromNylasWebhookPayloadTransformer implements TransformerInterface<TypeNylasMessageWebhookPayload, TypeMessageThread>
{
	/**
	 * Constructor.
	 *
	...
	public constructor(
		...
		private schema: ZodSchema = messageThreadSchema
	) {}
	 ...
	 * @returns {TypeMessageThread} - The transformed chat object.
	 */
	public transform(webhookPayload: TypeNylasMessageWebhookPayload): TypeMessageThread
	{
		const email = webhookPayload.data.object;

		const messages: TypeMessage[] = this.transformToMessages(email);
	...
	/**
	 * Transforms the Nylas email into a list of TypeMessage objects.
	 *
	 * @param {TypeNylasEmail} email - The Nylas email object.
	 * @returns {TypeMessage[]} - THe list of messages.
	 */
	private transformToMessages(email: TypeNylasEmail): TypeMessage[]
	{
		...
		// Map the attachments
		const attachments: TypeMessageAttachment[] = this.transformAttachments(email.attachments, email.id ?? '');

		// Create the message object
		const topMessage: TypeMessage = {
			id: email.id, // Use id from Nylas email as the reference ID for the message
			grantId: email.grant_id,
			body: fragments.length > 0 ? fragments[0].text.trim() : '', // First fragment will be the top most message
			attachments: attachments,
			date: email.date,
			from: email.from
		};

		const messages: TypeMessage[] = [topMessage];
	 ...
	 * @returns {TypeMessageAttachment[]} - The transformed attachment objects.
	 */
