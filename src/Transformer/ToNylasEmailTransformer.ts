...
import type { TypeEmailMessage, TypeMessageAddress, TypeMessageAttachment } from '@dmitryrechkin/message-core';
...
export class ToNylasEmailTransformer implements TransformerInterface<TypeEmailMessage, TypeNylasEmail>
{
	...
	public transform(input: TypeEmailMessage | undefined): TypeNylasEmail | undefined
	{
		...
		return {
			...
			date: input.sendAt || Math.floor(new Date().getTime() / 1000),
	...
	/**
	 * Transforms an array of TypeMessageAddress objects to an array of TypeNylasEmailAddress objects.
	 *
	 * @param {TypeMessageAddress[] | undefined} addresses - The custom message addresses
	 * @returns {TypeNylasEmailAddress[]} - The transformed Nylas email addresses
	 */
	private transformAddresses(addresses?: TypeMessageAddress[]): TypeNylasEmailAddress[]
	{
		if (!addresses)
		{
			return [];
	 ...
	 */
	private transformAttachments(attachments?: TypeMessageAttachment[]): TypeNylasEmailAttachment[]
	{
		if (!attachments)
		{
			return [];
